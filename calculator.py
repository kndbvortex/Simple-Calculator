...
class Calculatrice(QMainWindow):
	...
	def __init__(self, *args, **kwargs):
		...
		self.ui.bouton_egal.clicked.connect(lambda: self.set_label_calcul(self.ui.label_operation.text()))
		...
		self.ui.comboBox_fonctions.currentIndexChanged.connect(lambda : self.gestion_fonction(self.ui.comboBox_fonctions.currentIndex()))
	...
	def insert_virgule(self):
		...
		if len(ecran ) == 0 :
			...
			if (not virgule_present and not op_present) :
				...
				self.ui.label_operation.setText(self.ui.label_operation.text() + "0.")	
	...
	def extraction_pour_calcul_post_fixe(self, chaine_caractere):
		...
		while i  < len(chaine_caractere):
			...
			#Si c'est un nombre 
				...
				val = ""
				while  i < len(chaine_caractere) and not (chaine_caractere[i] in [k for k in self.liste_operateurs ] + ["(", ")"]) : 
	...
	def set_label_calcul(self, chaine):
		...
		self.ui.label_resultat.setText( str(self.calcul_post_fixe(liste_charac)))
...
def main():
	...
	#Shunting Yard Algorithm
def transform_post_fixe(items):
	operateurs = []
	liste_final = deque()
	for element in items:
		if element in liste_operateurs:
			print ("ok ......",operateurs)
			if not operateurs :
				operateurs.append(element)
			elif operateurs[-1] == "(" :
				...
				while operateurs :
					print(liste_final,operateurs)
					a = (operateurs[-1] in liste_operateurs)
					if a : 
						if (liste_operateurs[element][0] <= liste_operateurs[operateurs[-1]][0] and liste_operateurs[element][1] == liste_operateurs[operateurs[-1]][1]) or \
							  (liste_operateurs[element][0] <= liste_operateurs[operateurs[-1]][0] and liste_operateurs[element][1] != liste_operateurs[operateurs[-1]][1]) :
							  liste_final.append(operateurs.pop())
						else :
							break
					else:
		...
		#Si c'est une parenthèse ouvrante on l'ajoute si c'est une parenthèse fermante on dépile les opérateurs jusqu'à la prochaine parenthèse ouvrante
			...
			if element == "(":
				...
				print("dépilage",liste_final ,operateurs, element)
				while operateurs:
					print("good .............",operateurs[-1])
					if operateurs[-1] != "(":
						print("good")
						liste_final.append(operateurs.pop())
					else:
...
def extraction_pour_calcul_post_fixe( chaine_caractere):
	...
	if len(chaine_caractere) == 0:	
		...
		for i in chaine_caractere :
			...
			if i not in  [k for k in liste_operateurs] + ["(" , ")"]: 	
				uniquement_fait_op = False;
				break
		if uniquement_fait_op:
			return ""

	if chaine_caractere[-1] in liste_operateurs:
		chaine_caractere = chaine_caractere[:-1]
	es_negatif = False
	#final est une liste qui contiendra l'opération post fixée
	final = []
	...
	while i  < len(chaine_caractere):
		# s'il s'agit d'une simple opération (+, -, /, *) on ajoute le caractère dans operateurs 
		if chaine_caractere[i] in liste_operateurs:
			if chaine_caractere[i] != "-" :
				final.append(chaine_caractere[i])
			else :
				if i == 0  or (i > 0 and chaine_caractere[i - 1] == "(" ):
		...
		#les s'il s'agit d'une parenthèse  
			...
			if chaine_caractere[i] == "(":
				if chaine_caractere[i-1] in liste_chiffres and i > 0:
					final.append("x")
				final.append("(")
			else:
				final.append(chaine_caractere[i])
		...
		#Si c'est un nombre 
			...
			#le nombre peut avoir plusieurs chiffres d'où la présence de cette variable var qui nous aidera à rassembler ceux-ci
			val = ""
			while  i < len(chaine_caractere) and not (chaine_caractere[i] in [k for k in liste_operateurs ] + ["(", ")"]) : 
				val = val + chaine_caractere[i]
				i = i + 1
...
def calcul_post_fixe(final):
		i = 0
		while i < len(final):
			if  final[i] in liste_operateurs:
				print("un operateur ...", final[i], final)
				a = final[i-2]
				b = final[i-1]
				a = calcul(a,b,final[i])
				b = i-2
				for j in range(3):
					final.pop(b)
